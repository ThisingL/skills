...
If there ARE staged changes, write the full diff to a temporary file and read it:

```bash
git diff --cached > .aicommit.tmp
```

Then use the Read tool to read `.aicommit.tmp`. This ensures large diffs are not truncated.
...
Analyze the recent commit history to detect the project's existing style:

- **Language**: Are descriptions in English or Chinese (or mixed)?
- **Scope usage**: Do most commits use scopes? What scope naming conventions?
- **Body usage**: Do recent commits tend to have bodies or are they single-line?
- **Capitalization**: First letter uppercase or lowercase?
- **Any other patterns**: Ticket references, emoji usage, etc.

This style analysis is an **implicit input** to generation — you will match the project's existing conventions automatically without asking the user.
...
2. **What scope is affected?** — Use the scope detection rules below (and match the scope style observed in git history)
...
### Step 4: Ask About Detail Level

Use `AskUserQuestion` to ask the user how detailed they want the commit message to be. Based on git history analysis, put the more likely choice first (marked as recommended):

- If history shows most commits have no body → put "简洁" first as recommended
- If history shows commits frequently have bodies → put "详细" first as recommended

```
? 提交信息详细程度：
- 简洁 — 仅一行标题，如: feat: 添加登录功能
- 详细 — 附带说明段落，解释改了什么、为什么改
```

(Swap the order and recommendation based on git history analysis above.)

### Step 5: Generate 3 Commit Messages
...
**Three options should have the same structure** (all with body, or all without body — based on the user's choice in Step 4), and **match the project's existing style** detected from git history (language, scope conventions, capitalization). They should **vary in angle and wording:**

- Different type interpretations when reasonable (e.g., the same change could be `refactor` or `fix`)
- Different scope choices (with scope vs without, or different scope nouns)
- Different description wording that emphasizes different aspects of the change
- If body is included: different explanations focusing on different aspects (the "what", the "why", the contrast with previous behavior)

The goal is to give the user **3 meaningfully different perspectives** on the same change, not 3 levels of verbosity.
...
The user may also select "Other" (built-in) to provide custom instructions, e.g., "太短了，需要更详细的描述" or "focus on the performance aspect". Treat their input as guidance and go back to Step 5, generating 3 new messages that follow their instructions.

If the user selects **Regenerate / 重新生成**, go back to Step 5 and produce 3 new different messages. Do not repeat previously generated options.
...
After the commit succeeds, clean up the temporary file:
...
- **DO NOT** modify any source files in the working tree (`.aicommit.tmp` is the only file you create, and it must be deleted after commit)
